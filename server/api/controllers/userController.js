...
export const enrollStudentInGroup = async (request, response) => {
    ...
    try {
        ...
        if (!group) {
            return response.status(404).json({ message: "GroupList not found" });
        }
        ...
        if (group.participants.length >= group.max_capacity) {
            return response.status(403).json({ message: "GroupList is full. User can be added to the waitlist" });
        }
